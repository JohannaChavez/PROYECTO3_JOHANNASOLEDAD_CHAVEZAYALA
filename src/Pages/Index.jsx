...
//import {costPropiedad} from "../../services/axios.config"
...
export default function Index() {
  ...
  const cotizarPoliza = () => {
    ...
    setData({...data, poliza:poliza.toFixed(2), fechaCotizacion:new Date().toLocaleDateString()});
    guardarHistorial();
  };

  const setFactorPropiedad = (e) => {
    setData({...data, factorPropiedad:e.target.value, propiedad:e.nativeEvent.target[e.nativeEvent.target.selectedIndex].text});
  };

  const setFactorUbicacion = (e) => {
    setData({...data, factorUbicacion:e.target.value, ubicacion:e.nativeEvent.target[e.nativeEvent.target.selectedIndex].text});
  };
